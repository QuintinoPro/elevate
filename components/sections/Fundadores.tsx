    ...
    <Section id="fundadores" stars className="border-y border-line/25 bg-surface/20">
      ...
      <div className="mt-14 grid gap-6 md:grid-cols-2">
        {fundadores.pessoas.map((p) => (
          <article key={p.nome} className="overflow-hidden rounded-2xl border border-line/30 bg-ink">
            ...
            {/* Bio de uma linha: aqui a foto é a prova, o texto só nomeia. */}
            <div className="p-7 pt-6 sm:p-9 sm:pt-6">
              <h3 className="text-2xl font-bold">{p.nome}</h3>
              <p className="mt-3 leading-relaxed text-paper/60">{p.bio}</p>

              <a
                href={p.instagram}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-5 inline-block text-sm text-paper/40 transition-colors hover:text-accent"
              >
                {p.handle}
              </a>
            </div>
