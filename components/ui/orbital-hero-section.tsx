...
export function OrbitalHeroSection({
  ...
  useEffect(() => {
    ...
    function drawSun(k: number, t: number) {
      ...
      // sits close around it. Every tint here is derived from sunColor, so a
      // brand palette carries all the way through the glare.
      const haze = ctx!.createRadialGradient(cx, cy, 0, cx, cy, R * 14);
      haze.addColorStop(0, `rgba(${r},${g},${b},${0.05 * k})`);
      haze.addColorStop(0.4, `rgba(${r},${g},${b},${0.014 * k})`);
      ...
      outer.addColorStop(0.3, `rgba(${r},${g},${b},${0.1 * k})`);
      outer.addColorStop(0.62, `rgba(${r},${g},${b},${0.025 * k})`);
      ...
      bloom.addColorStop(0.42, `rgba(255,255,255,${0.7 * k})`);
    ...
    function render(t: number) {
      ...
      if (C.showSunTrack && !C.starsOnly) {
        ...
        const [sr, sg, sb] = parseRGB(C.sunColor);
        project(0, 0, 0);
        const hx = P.x, hy = P.y;
        ...
        if (P.ok) {
          ...
          grad.addColorStop(0.45, `rgba(${sr},${sg},${sb},${0.55 * k})`);
